  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 md:px-8 flex justify-between items-center">
          ...
          <div className="hidden lg:flex items-center gap-6">
            {navItems.map(item => (
              <div
                key={item.name}
                className={`relative ${comingSoon && 'pointer-events-none'}`}
              >
                <Link
                  href="#"
                  onClick={(e) => e.preventDefault()}
                  className="text-sm font-bold uppercase text-white/50 flex items-center gap-1 cursor-not-allowed"
                >
                  {item.name}
                  {item.children && (
                    <ChevronDown size={14} />
            ...
            {/* HUBUNGI (DISABLED) */}
            <div className="px-6 py-2.5 rounded-full bg-secondary-500/40 font-bold text-primary-900/60 cursor-not-allowed">
              Hubungi
            </div>
          </div>
      ...
      >
        ...
        <div
          className={`absolute top-0 right-0 h-full w-3/4 max-w-sm
          bg-primary-900 transition-transform duration-500
          ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
        >
          {/* CLOSE */}
          <button
          ...
          <div className="pt-24 px-6 text-white">
            {navItems.map(item => (
              <div key={item.name} className="border-b border-white/10">
                <div className="py-4 text-lg font-bold uppercase text-white/50 cursor-not-allowed">
                  {item.name}
                </div>
              </div>
            ))}
            ...
            <div className="mt-8 pb-10">
              <div
                className="block w-full text-center py-4 
                bg-secondary-500/40 text-primary-900/60 
                rounded-full font-bold cursor-not-allowed"
              >
                <Phone size={18} className="inline mr-2" />
                Hubungi Admin
              </div>
